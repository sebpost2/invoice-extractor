      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg">
          ...
          <div className="flex items-center gap-3 mt-3">
            <p className="text-sm text-blue-100">
              {t.dashboard.receiptsThisMonth(thisMonth.count)}
            </p>
            <ChangeBadge
              current={thisMonth.totalSpent}
              previous={lastMonth.totalSpent}
              suffix={t.dashboard.vsLastMonth}
              newLabel={t.dashboard.newSuffix}
              variant="onAccent"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <KPI
            ...
            primary={formatMoney(allTime.totalSpent, allTime.currency)}
          />
          <KPI
            label={t.dashboard.avgPerReceipt}
            primary={formatMoney(allTime.avg, allTime.currency)}
            secondary={t.dashboard.totalReceiptsCount(allTime.count)}
          />
          <KPI
            label={t.dashboard.topVendor}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-zinc-900 rounded p-4">
            <h2 className="text-xs uppercase tracking-wide text-zinc-500 mb-3">
              {t.dashboard.chartByVendor}
            </h2>
            <VendorPieChart
              data={vendorData}
              emptyLabel={t.dashboard.chartEmptyVendors}
            />
          </div>
          <div className="bg-zinc-900 rounded p-4">
            <h2 className="text-xs uppercase tracking-wide text-zinc-500 mb-3">
              {t.dashboard.chartByMonth}
            </h2>
            <MonthlyBarChart
              data={monthlyData}
              emptyLabel={t.dashboard.chartEmptyDates}
            />
          </div>
        </div>
      </div>
...
function KPI({
  ...
  return (
    <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
      ...
      <div className="text-xl font-semibold mt-1 truncate" title={primary}>
        {primary}
      </div>
      {secondary && (
...
function computeVendors(receipts: Receipt[], othersLabel: string) {
  ...
  if (restTotal > 0) {
    top.push({ name: othersLabel, total: Math.round(restTotal * 100) / 100 });
  }
  return top;
}
